 ...
 client.ev.on('call', async (callData) => {
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      console.log(color("Congrats, 𝗖𝗢𝗢𝗟-𝗞𝗜𝗗 𝗫𝗠𝗗 has successfully connected to this server", "green"));
      console.log(color("Follow us on whatsapp on https://whatsapp.com/channel/0029VarDt9t30LKL1SoYXy26", "red"));
      console.log(color("Text the bot number with menu to check my command list"));
      const Texxt = `*╭═══════❖•ೋ° °ೋ•❖══════╮*\n`+`┊𓅂 𝗖𝗼𝗻𝗻𝗲𝗰𝘁𝗲𝗱 » »【𝗖𝗢𝗢𝗟-𝗞𝗜𝗗 𝗫𝗠𝗗】\n`+`┊☆ 𝗠𝗼𝗱𝗲 »» ${mode}\n`+`┊☆ 𝗣𝗿𝗲𝗳𝗶𝘅 »» ${prefix}\n`+`*╰═══════❖•ೋ° °ೋ•❖══════╯*`
